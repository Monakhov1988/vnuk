...
- Команды:
  ...
  - `/help` — полный список 20+ тем с эмодзи-иконками и примерами фраз (рецепты, погода, кино, загадки, рукоделие, молитвы, доставка, техника и т.д.)
  - `/link <код>` — привязка Telegram к аккаунту родителя по linkCode
  - `/register <имя>` — регистрация нового аккаунта родителя прямо из Telegram (+ подсказка про /help)
