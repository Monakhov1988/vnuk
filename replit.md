...
- Команды:
  ...
  - `/register <имя>` — регистрация нового аккаунта + onboarding (город → интересы) + persistent keyboard
  - **Persistent ReplyKeyboard** (4 кнопки, всегда видна внизу): ⚙️ Настройки | 💊 Здоровье | 🏠 Хозяйство | 🎭 Афиша
  - **Категории → InlineKeyboard подменю**:
    - 💊 Здоровье: Лекарства, Давление, Молитва
    - 🏠 Хозяйство: Погода, Рецепт, Фото счётчика, Огород
    - 🎭 Афиша: Что по ТВ, Что в кино, Загадка, Стихотворение, Открытка
    - ⚙️ Настройки: меню с "Темы и экспертиза" + "Личность бота"
...
- **TTS (Text-to-Speech)**: tts-1 (голос nova, формат opus) — верное воспроизведение текста без генеративного дрифта. STT — whisper-1 с verbose_json и quality gate (low/medium/high confidence)
    ...
    - `search_transport(from, to, date?, transport_type?)` — расписание поездов/электричек/автобусов через Perplexity: рейсы, время отправления/прибытия, время в пути, ориентировочная стоимость. Источники: РЖД, Яндекс.Расписания, Туту.ру. Ссылки на покупку билетов в конце
    - `search_clinic(query, city?)` — поиск врачей и клиник через Perplexity: рейтинг, отзывы пациентов, стоимость приёма, адрес, телефон. Источники: ПроДокторов, ДокДок, СберЗдоровье. Ссылки на запись в конце
    - `search_medicine(query)` — информация о лекарствах через Perplexity: инструкция, побочные эффекты, противопоказания, аналоги, цены. Источники: vidal.ru, rlsnet.ru, apteka.ru. Обязательное предупреждение о консультации с врачом
    ...
    - `search_gov_services(query)` — пенсии, льготы, субсидии, госуслуги через Perplexity. Источники: gosuslugi.ru, sfr.gov.ru, consultant.ru. Актуальные суммы и порядок оформления
    - `search_garden(query, region?)` — садоводство/огород через Perplexity с учётом текущего месяца и региона. Источники: antonovsad.ru, 7dach.ru, botanichka.ru
    - `search_product(query)` — товары с ценами и отзывами через Perplexity. Источники: Яндекс.Маркет, Ozon, Wildberries. Ссылки на маркетплейсы
