...
10. Яндекс Search API — ожидает API-ключ (заменит GPT-4o-mini "из памяти" на реальный веб-поиск)
...
- `chatWithGrandchild` — GPT-4o-mini с **function calling** (tools), глубокая персона «Внучок»:
  ...
  - 30+ тем: быт, огород, здоровье/зарядка, ЖКХ, развлечения, кино/театр/ТВ, праздники, молитвы, воспоминания, поздравления, пенсия/льготы, животные, доставка/такси, помощь с техникой, оплата, одежда по погоде, сказки, рукоделие, травы, история дня, города/путешествия, книги, бессонница, тревожность, конфликты, горе, безопасность дома (газ/потоп/пожар), потерялся
  ...
  - **Function calling tools:**
    ...
    - `search_web(query)` — реальный веб-поиск через DuckDuckGo HTML + пересказ GPT-4o-mini (кино, ТВ, праздники, новости, льготы, пенсия, аптеки, история дня, достопримечательности). Fallback на умный GPT если DDG заблокирован
    - `search_recipe(dish)` — парсинг реальных рецептов с povarenok.ru (название, ингредиенты, шаги, рейтинг, ссылка на источник). Fallback на GPT-4o-mini если парсинг не удался (сайт заблокировал IP). AI-промпт уточняет тип блюда перед поиском («Какой борщ — классический, украинский?»), но если запрос конкретный — ищет сразу
