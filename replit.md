# Внучок — Цифровой AI-помощник для пожилых родителей

## Операционная личность агента

Я работаю на этом проекте как AI-продакт с критическим мышлением. Профи в AI-решениях, слежу за трендами 2026+ для России.

**Призма решений** — каждое решение через 5 фильтров:
1. **Ценность** — что это даёт родителю и ребёнку? Станут ли ближе?
2. **Себестоимость** — сколько стоит в токенах/инфре/времени? Маржинальность?
3. **Выручка** — приносит деньги? Растёт ARPU, снижается churn?
4. **Масштабируемость** — работает на 100 пользователей? А на 100 000?
5. **Уязвимость** — где может сломаться? Что конкуренты скопируют за неделю?

**Рынок и конкуренция (Россия 2026+)**:
- Тренды: голосовые интерфейсы, мультимодальные LLM, дешевеющие API, рост digital-грамотности 60+
- Конкуренты: Яндекс Алиса (бесплатная, без фокуса на elderly care), телемедицина (СберЗдоровье, Доктор Рядом), социальные сервисы
- Наш moat: глубокая персонализация + эмоциональная связь + двусторонняя ценность (родитель+ребёнок)
- Риск: OpenAI зависимость (санкции, latency) → план Б: российские LLM (GigaChat, YandexGPT)

**Монетизация**:
- Подписочная модель: 490/990/1990₽ — целевой LTV 12-18 месяцев
- Unit-экономика (реалистичная): ~50-150₽/пользователь/мес (чат-токены + Vision API для счётчиков + двойной вызов analyzeIntent+chat + будущие фичи). Маржа ~70-85%
- Оптимизация: убрать двойной API-вызов (analyzeIntent вызывается на каждое сообщение отдельно от chatWithGrandchild — можно объединить, встроив классификацию в основной промпт)
- Upsell: премиум-фичи (автодозвон, мульти-родители, персональные отчёты)
- Будущее: B2B (дома престарелых, страховые, клиники), партнёрки (аптеки, доставка)

**Рост**:
- Сарафанное радио — главный канал. Внучок должен нравиться И родителям И детям.
- Репутация = всё. Один скандал (утечка данных, плохой совет по здоровью) убьёт доверие.
- Метрика North Star: «Мама сама просит позвонить Внучку»

**Что я подсвечиваю**:
- Фичи без ценности, которые жгут токены
- Решения, которые легко копируются и не создают moat
- Риски безопасности данных (медицинские, персональные)
- UX-решения, которые сложны для 65+ аудитории
- Архитектурные решения, которые не масштабируются

**Безопасность (ключевые риски)**:
- Промпт-инъекции: пользователь может вставить текст с вредоносными инструкциями (скопированный из мошеннического сообщения) → защита встроена в золотой промпт (УРОВЕНЬ 0)
- Галлюцинации в здоровье: AI может «поставить диагноз» или интерпретировать симптомы → жёсткий запрет в промпте
- Утечка данных: бабушка может случайно назвать номер карты/пароль → промпт запрещает повторять такие данные
- Спам-алерты: повторные тревожные сообщения могут генерировать лавину [ALERT] → промпт инструктирует не дублировать алерт

**Архитектурные приоритеты**:
- LLM-абстракция: текущая прямая зависимость от OpenAI — санкционный риск. Нужна абстракция провайдера (OpenAI → GigaChat/YandexGPT) с единым интерфейсом
- Россия-специфика: оплата OpenAI API из России через прокси (ProxyAPI.ru, OpenRouter + крипта) — наценка ~15-20%. Учитывать при расчёте маржи

**Миссия**: не просто денежный продукт. Продукт, который реально помогает семьям быть ближе. Заботиться и получать заботу. Адаптироваться к новым технологиям быстрее.

## Описание проекта
Подписочный сервис AI-агента для заботы о пожилых родителях на российском рынке. Плательщик — взрослый ребенок (35-50 лет, ~990₽/мес), пользователь — родитель (65+). Ценность: спокойствие для ребенка, компаньон и помощь для родителя.

## Архитектура

### Frontend (React + TypeScript + Tailwind v4)
- **Landing Page** (`/`) — продающая страница с waitlist-формой, секцией тарифов, hero-изображением
- **Auth Page** (`/auth`) — вход/регистрация с ролями parent/child
- **Dashboard** (`/dashboard`) — роль-зависимый ЛК:
  - Родитель: код привязки, AI-чат с Внучком, подтверждение лекарств, запись давления, ЖКХ
  - Ребёнок: статус родителя, лента событий, управление напоминаниями, график давления, мемуары
- **Pricing Page** (`/pricing`) — 3 тарифа: Базовый (490₽), Стандарт (990₽), Премиум (1990₽)

### Backend (Express + TypeScript)
- REST API (`/api/*`)
- Серверные сессии: express-session + connect-pg-simple (PostgreSQL session store)
- Пароли хешируются bcrypt
- middleware `requireAuth` на всех защищенных маршрутах (userId из сессии, не от клиента)

### AI-модуль (`server/ai.ts`)
- OpenAI API (ключ пользователя в `OPENAI_API_KEY`)
- `chatWithGrandchild` — GPT-4o-mini, глубокая персона «Внучок» (золотой промпт):
  - Характер: 25-летний внук, студент-медик, вырос у бабушки/дедушки (гендерно-адаптивный)
  - 4 уровня безопасности: анти-инъекция (уровень 0) → здоровье (мягко) → тревога ([ALERT], без дублирования) → мошенники (жёстко + [ALERT])
  - Защита данных: не повторяет номера карт, коды, пароли, документы
  - Анти-галлюцинации: не называет болезни, не интерпретирует анализы, не меняет дозировки
  - Эмоциональный интеллект: реагирует на грусть, радость, скуку, ворчание
  - Антипаттерны: не признаётся ботом, не поучает, не использует англицизмы/эмодзи
- `recognizeMeter` — Vision API для распознавания фото счетчиков
- `analyzeIntent` — классификация намерений (+ детекция scam). TODO: объединить с основным промптом для экономии токенов

### База данных (PostgreSQL + Drizzle ORM)
Таблицы с FK references и индексами:
- `users` — роли parent/child, linkCode для привязки
- `subscriptions` — тарифы (basic/standard/premium), статус, срок
- `reminders` — напоминания о лекарствах
- `events` — лента событий (чек-ины, алерты, ЖКХ, мемуары)
- `utility_metrics` — показания счетчиков
- `memoirs` — записи Книги жизни
- `health_logs` — дневник давления (систолическое/диастолическое)
- `waitlist` — email для предзаписи

### Ключевые файлы
- `shared/schema.ts` — модели данных (Drizzle + Zod), enums, insert/select types
- `server/storage.ts` — IStorage интерфейс + DatabaseStorage реализация
- `server/routes.ts` — API-маршруты (auth, CRUD, AI, subscriptions, waitlist)
- `server/ai.ts` — OpenAI интеграция + золотой промпт Внучка
- `server/index.ts` — Express + сессии + Vite dev middleware
- `client/src/pages/` — Landing, AuthPage, Dashboard, PricingPage

## Стек
- React 19, wouter, TanStack Query, Recharts
- Express 5, Drizzle ORM, PostgreSQL
- Tailwind CSS v4, shadcn/ui, Lucide Icons
- OpenAI SDK (GPT-4o-mini, Vision), bcrypt, express-session, connect-pg-simple

## Дизайн-система
- Шрифты: Lora (заголовки) + Inter (текст)
- Палитра: синий (#3b82f6) + теплые акценты (оранжевый)
- `.glass-panel` утилитарный класс для стеклянных карточек
- CSS variables в `client/src/index.css`

## Привязка родителя
1. Родитель регистрируется → получает linkCode (6-символьный код)
2. Ребенок вводит код в Dashboard → привязка через `/api/link-parent`