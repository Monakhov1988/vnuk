...
- `chatWithGrandchild` — GPT-4o-mini с **function calling** (tools), глубокая персона «Внучок»:
  ...
  - **Function calling tools:**
    ...
    - `search_recipe(dish)` — GPT-4o-mini генерирует рецепт + ссылка на eda.ru. Перед поиском код-уровневый перехват в telegram.ts/routes.ts: если блюдо из RECIPE_CLARIFICATIONS (борщ, суп, пирог и др.) — бот уточняет вариант напрямую без AI. Tool description также запрещает вызов с общими названиями
