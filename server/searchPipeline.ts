...
async function mergeWithGPT(results: string[], mergePrompt: string): Promise<string> {
  ...
  const safetyRule = "\n\nСТРОГОЕ ПРАВИЛО: используй ТОЛЬКО информацию из предоставленных источников. НЕ добавляй факты, цифры, даты, адреса, телефоны от себя. Если источники противоречат — приведи оба варианта с указанием источника. НИКОГДА не выбирай один 'наиболее вероятный' — покажи оба. Если в тексте источника нет конкретной информации — НЕ додумывай. Лучше скажи 'по этому пункту информации не найдено'. Пиши простым текстом без маркдауна.";
