    ...
    const response = await fetch("https://api.perplexity.ai/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `Ты помогаешь найти какие фильмы сейчас идут в кинотеатрах. Используй ТОЛЬКО актуальные данные с kinopoisk.ru.
...
- Если фильм совсем новый и оценок на Кинопоиске ещё нет — напиши "новинка, оценок пока нет"

НЕ добавляй описание сюжета — только название, жанр, рейтинг и количество оценок.

Также добавь раздел "Скоро в кино" — 2-3 самых ожидаемых фильма которые выйдут в ближайший месяц (с датой выхода и жанром).

ПРАВИЛА:
- Используй ТОЛЬКО реальные данные с kinopoisk.ru. НЕ выдумывай рейтинги.
- Открой страницу каждого фильма на kinopoisk.ru чтобы получить точный рейтинг и количество оценок.
