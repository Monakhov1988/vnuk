...
🎙 Голосовые — просто наговорите!`;
  ...
  bot.on("message:voice", async (ctx) => {
    ...
    try {
      ...
      const lowerVoice = userText.toLowerCase().trim().replace(/[?.!,]$/g, "");
      const voiceDishBase = lowerVoice.replace(/^рецепт\s+/, "").trim();
      const DISH_FORMS_VOICE: Record<string, string> = {
      ...
      const normalizedVoiceDish = DISH_FORMS_VOICE[voiceDishBase] || voiceDishBase;
      const VOICE_RECIPE_CLARIFICATIONS: Record<string, string> = {
      ...
      const voiceClarification = VOICE_RECIPE_CLARIFICATIONS[normalizedVoiceDish];
