...
async function registerUserFromTelegram(chatId: string, name: string): Promise<{ message: string; userId: number }> {
  ...
  return {
    message:
      ...
      `Давайте познакомимся поближе — расскажите немного о себе!\n\n` +
...
async function handleOnboardingStep(chatId: string, userText: string, ctx: any): Promise<boolean> {
  ...
  if (state.step === "interests") {
    ...
    await ctx.reply(
      `Отлично, я запомнил! 📝\n\n` +
      `Ну вот и познакомились! Теперь можем общаться каждый день — про погоду, рецепты, здоровье, да обо всём на свете.\n` +
      `Просто напишите мне или нажмите кнопку внизу — я всегда рядом!`,
...
export async function startTelegramBot() {
  ...
  bot.command("start", async (ctx) => {
    ...
    if (deepLinkCode) {
      ...
      if (codeOwner.role === "child") {
        ...
        await ctx.reply(
          ...
          `Меня зовут Внучок — буду рад общаться каждый день!\n\n` +
    ...
    await ctx.reply(
      `Привет! Я — Внучок 😊\n\n` +
      `Буду рад общаться каждый день! Я могу:\n` +
      `🌤 Рассказать, какая погода за окном\n` +
      `🍳 Найти вкусный рецепт\n` +
      `💊 Напомнить про лекарства\n` +
      `🚆 Подсказать расписание электричек\n` +
      `⚖️ Разобраться с льготами и пенсией\n` +
      `📖 Почитать стихотворение\n` +
      `...и просто поболтать!\n\n` +
      `Можете писать мне или говорить голосом 🎤 — я всё пойму.\n\n` +
      `Давайте знакомиться — нажмите кнопку:`,
  ...
  bot.on("message:text", async (ctx) => {
    ...
    if (!user) {
      ...
      await ctx.reply(
        `Привет! Я — Внучок 😊\n` +
        `Погода, рецепты, лекарства, льготы, электрички — и просто поболтать!\n\n` +
        `Давайте знакомиться — нажмите кнопку:`,
        { reply_markup: keyboard }
      );
      return;
