...
export function detectIntentLocal(text: string, hasAlert: boolean): string {
  ...
  const falsePositivePatterns = /(?:умира[юе]\s+(?:от\s+)?(?:смех|хохот|скук|голод|любопытств|нетерпени|жар[ыа]|завист)|убийственно\s+(?:вкусн|красив|интересн)|(?:сдохну|умру)\s+(?:от\s+)?(?:смех|скук|жар)|(?:помру|сдохну)\s+со\s+смех|(?:ржу|хохочу)\s+(?:не могу|до слёз)|шутк[аиу]|(?:^|\s)смеш|(?:упала|падала)\s+(?:от|со)\s+смех|(?:помогите|спасите)\s+(?:от\s+)?(?:смех|скук|кот|соседк|внук)|(?:инфаркт|инсульт|умру)\s+(?:от\s+)?(?:смех|красот|вкуснот|счасть)|сердце\s+(?:замирает|останавливается)\s+(?:от\s+)?(?:красот|счасть|любв|радост|восторг)|(?:голова\s+кружится|кружится\s+голова)\s+от\s+(?:счасть|радост|любв|восторг|запах|аромат|красот))/;
