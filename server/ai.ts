...
import { getWeather, searchWeb, searchRecipe, generateImage, searchMovie, searchPlace, searchTransport, searchClinic, checkSearchRateLimit, sanitizeWebContent } from "./tools";
...
ЗДОРОВЬЕ: напомнить выпить таблетки, спросить про давление, посоветовать измерить. Знаешь базовую первую помощь. Можешь подсказать простые упражнения и зарядку для пожилых (разминка шеи, вращение плечами, ходьба на месте, дыхательная гимнастика). Но НЕ ставишь диагнозов. НЕ называешь конкретных лекарств. НЕ интерпретируешь симптомы и результаты анализов. НЕ предлагаешь менять дозировку. НЕ называешь болезни. Говоришь: «Это лучше с врачом обсудить.» Если спрашивают про запись к врачу — вызови search_clinic чтобы найти врача с хорошими отзывами. Если спрашивают про аптеку — используй search_web.
...
const RETRYABLE_TOOLS = new Set(["search_web", "search_recipe", "get_weather", "search_movie", "search_place", "search_transport", "search_clinic"]);
...
ВАЖНО: Если собеседник спрашивает о чём-то, чего НЕТ в этом списке (имена, события, факты) — ты этого НЕ ЗНАЕШЬ. НЕ ВЫДУМЫВАЙ! Скажи: "Ой, запамятовал... Напомни мне?" Лучше честно переспросить, чем назвать чужое имя.`;
  ...
  while (iterations < MAX_TOOL_ITERATIONS) {
    ...
    for (const toolCall of choice.message.tool_calls) {
      ...
      const isSearchCall = fnName === "search_web" || fnName === "search_recipe" || fnName === "search_movie" || fnName === "search_place" || fnName === "search_transport" || fnName === "search_clinic";
      if (isSearchCall) {
        searchCallsThisResponse++;
