...
export function detectIntentLocal(text: string, hasAlert: boolean): string {
  ...
  const heartPatterns = /(?:давит (?:в |за )?груд|жжёт (?:в |за )?груд|жж[её]т (?:в |за )?груд|печ[её]т в груд|боль отда[её]т в (?:рук|лопатк|челюст|шею|спин)|сердце (?:давит|сжимает|колет|болит|прихватил|схватил|останавливается|замирает|выскакивает)|холодный пот|(?:тошнит|рвёт).{0,20}(?:боль|груд|сердц)|(?:груд|сердц).{0,20}(?:тошнит|рвёт)|аритми[яи]|сердце (?:бьётся|колотится|стучит)\s+(?:сильно|быстро|нерегулярн)|(?:сердце|в груди)\s+(?:замерло|замирает|остановилось|пропускает)|(?:тяжесть|тяжело)\s+(?:в\s+)?(?:груди|за грудиной)|(?:сжимает|стягивает|сдавливает)\s+(?:грудь|грудную|за грудин)|(?:жар|горит|печёт)\s+(?:в\s+)?(?:груди|за грудин)|(?:стенокарди|грудная жаба|приступ).{0,15}(?:сердц|серд|груд)|нитроглицерин\s+не\s+(?:помогает|помог|действует)|(?:пульс|сердце)\s+(?:\d{3,}|(?:больше|за|выше)\s+(?:ста|100|120|130|140|150))|(?:резкая|острая|сильная|невыносимая)\s+боль\s+(?:в\s+)?(?:груди|сердц|за грудин)|(?:задыхаюсь|не хватает воздух|не могу вздохнуть|тяжело дышать).{0,20}(?:сердц|груд|боль))/;
  ...
  const poisoningPatterns = /(?:отравил[ао]сь|отравлени[ея]|(?:съел[аи]?|выпил[аи]?).{0,15}(?:испорченн|просроченн|пропавш|тухл|плесен|несвеж)|(?:рвёт|рвота|понос|диарея).{0,15}(?:не прекращается|уже \d+ (?:час|раз)|целый день|без остановки|с кровью)|(?:выпил[аи]?|глотнул[аи]?|попил[аи]?).{0,15}(?:уксус|белизн|хлорк|бытов|средств|жидкост|химию|антифриз|растворител|бензин|керосин))/;
