...
${h.name} — ${dayWord}! Упомяни его тепло и естественно. Можно спросить как планируют отмечать, поздравить, поделиться тёплыми словами. НЕ будь формальным — как внук, а не открытка.`;
          ...
          const seasonTips: Record<string, string> = {
            ...
            "весна": "Порадуйся вместе: «Уже вовсю тает! Скоро на дачу?» или «Птички поют — весна пришла!»",
...
ГЛАВНОЕ — сначала забота, потом функция вскользь. НЕ делай функцию единственным содержанием сообщения. Если не получается вплести естественно — просто спроси как дела, без функции.`;
      ...
      case "gratitude": {
        if (ctx.daysSinceLastChat >= 7) {
          categoryInstruction = `ЗАДАЧА: Тёплое сообщение после долгого перерыва.
Давно не общались. НЕ акцентируй сколько дней прошло — это может вызвать чувство вины. Просто напиши мягко: «Привет, родная! Всё хорошо? Просто подумал о тебе» или «Привет! Как ты там? Вспомнил и решил написать». БЕЗ укора, БЕЗ «давно не общались». Просто тепло и ненавязчиво.`;
